    ...
    //   console.log("===================助力省钱大赢家===================");
      ...
      if (newShareCodes && newShareCodes.length > 0) {
        ...
        for (var i = 0; i < newShareCodes.length; i++) {
          redEnvelopeId = await getShareCodeInfo1(newShareCodes[i]);
          if (redEnvelopeId) {
...
async function redPacket() {
  return new Promise((resolve) => {
    ...
    $.get(
      taskGetUrl("spring_reward_receive", { inviter, linkId: "AkOULcXbUA_8EAPbYLLMgg" }),
      async (err, resp, data) => {
        try {
          if (err) {
...
async function springRewardQuery() {
  return new Promise((resolve) => {
    $.get(
      taskGetUrl("spring_reward_query", { linkId: "AkOULcXbUA_8EAPbYLLMgg", inviter: "" }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                if (
                  ...
                  console.log(helpType == "1" ?  `助力省钱大赢家助力失败;` : "助力省钱大赢家提现失败", data.data.helpResult.code, data.data.helpResult.errMsg);
                  if(data.data.helpResult.code==16005){
                    finishShareCode(shareCode)
                  }
                } else {
                  console.log(helpType == "1" ?  `助力省钱大赢家助力失败;` : "助力省钱大赢家提现失败", data);
                }
...
function exchange() {
  return new Promise((resolve) => {
    var result = {}
    $.get(
      taskGetUrl("exchange", {"linkId":linkId,"rewardType":2}),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                ...
                finishShareCode(null, $.UserName, shareCodeType)
              } else {
                console.log(data.errMsg);
                console.log(
...
function doTask(itemId) {
  return new Promise((resolve) => {
    var result = {}
    $.get(
      taskGetUrl("apDoTask", {"linkId":linkId,"taskType":"BROWSE_CHANNEL","taskId":139,"channel":4,"itemId":itemId,"checkVersion":true}),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function getTaskList() {
  return new Promise((resolve) => {
    var result = {}
    $.get(
      taskGetUrl("apTaskDetail", {"linkId":linkId,"taskType":"BROWSE_CHANNEL","taskId":139,"channel":4}),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
